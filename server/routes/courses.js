...
// GET /api/courses/:id/export-csv — Streamed Course Roster Attendance CSV Export
router.get('/:id/export-csv', authenticate, requireRole('teacher'), async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=attendance_course_${course.courseCode}.csv`);
    res.write('Roll Number,Student Name,Student Email,Status,Time (IST),Distance (m),Generated By\n');

    const cursor = Attendance.find({ courseId: course._id })
    ...
    for await (const row of cursor) {
      ...
      const email = row.studentId?.email ? `"${row.studentId.email.replace(/"/g, '""')}"` : '"N/A"';
      const time = row.timeString || 'N/A';
      const dist = row.distanceMeters != null ? `${row.distanceMeters}m` : 'N/A';

      res.write(`${roll},${name},${email},${row.status},"${time}","${dist}",${row.generatedBy}\n`);
    }
